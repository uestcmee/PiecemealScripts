...
import pandas as pd
import sys
pdf_dir="C:\\Wind\\Wind.NET.Client\\WindNET\\users\\W47779091884\\NA\\法律意见书"
...
for index,code in enumerate(code_list):
    if index<=57:
        continue
    if pdfs.get(code)==None:
        print('第{}个文件，该股票{}无相关文件'.format(index,code))
        continue
    print('第{}个文件，打开文件{}'.format(index,pdfs[code]))
    os.system(pdf_dir+'\\'+pdfs[code])
    # input('是否已经完成？') # 可以不用，因为os命令需要edge退出才算执行结束